...
with out_col2:
   ...
   # ★★★ [수정된 부분] 성간 소광 오차 (0.5가 아닐 때 동적으로 모두 잡아냄!)
    if not is_ext_correct:
        if user_extinction < 0.5:
            st.error(f"❗ [오차 원인 2] 성간 소광량 보정치(A={user_extinction})가 실제보다 작습니다. 우주 공간의 티끌 때문에 별빛이 흐려진 것을 충분히 보정하지 않으면, 별이 단순히 '멀리 있어서 어두운 것'으로 착각하게 되어 은하의 거리가 실제보다 더 멀게 왜곡됩니다.")
        else:
            st.error(f"❗ [오차 원인 2] 성간 소광량 보정치(A={user_extinction})가 실제보다 과도하게 입력되었습니다. 성간 소광을 실제보다 너무 크게 보정하면 별의 원래 밝기를 과대평가하게 되므로, 은하의 거리가 실제보다 너무 가깝게 계산되는 오차가 발생합니다.")
