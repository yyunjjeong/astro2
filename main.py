...
with out_col2:
    ...
    # [오차 원인 1]
    if "Ⅰ족" in user_pop:
        st.error("❗ [오차 원인 1] 실제보다 별을 '금속이 많은 젊은 별(Ⅰ족)'로 잘못 가정했습니다. 이 경우 별의 실제 절대등급(원래 밝기)을 너무 밝게 추정하게 되므로, 이론선에 맞추다 보면 은하의 거리가 실제보다 훨씬 멀리 있는 것으로 왜곡됩니다.")
    
   # ★★★ [수정된 부분] 성간 소광 오차 (0.5가 아닐 때 동적으로 모두 잡아냄!)
    if not is_ext_correct:
        if user_extinction < 0.5:
            st.error(f"❗ [오차 원인 2] 성간 소광량 보정치(A={user_extinction})가 실제(0.5)보다 작습니다. 우주 공간의 티끌 때문에 별빛이 흐려진 것을 충분히 보정하지 않으면, 별이 단순히 '멀리 있어서 어두운 것'으로 착각하게 되어 은하의 거리가 실제보다 더 멀게 왜곡됩니다.")
        else:
            st.error(f"❗ [오차 원인 2] 성간 소광량 보정치(A={user_extinction})가 실제(0.5)보다 과도하게 입력되었습니다. 성간 소광을 실제보다 너무 크게 보정하면 별의 원래 밝기를 과대평가하게 되므로, 은하의 거리가 실제보다 너무 가깝게 계산되는 오차가 발생합니다.")
        
    # ★★★ [추가된 부분] 거리가 틀렸을 때 나타나는 오차 원인 3번 메시지!
    if not is_dist_correct:
        st.error(f"❗ [오차 원인 3] 현재 예측된 은하의 거리({user_distance_kpc} kpc)가 관측 데이터와 물리적으로 일치하지 않습니다. 세페이드 변광성의 '주기-광도 관계 공식'에 의한 절대 등급과 겉보기 등급 사이의 거리 지수($m-M$) 균형이 맞지 않아 그래프의 빨간 선이 어긋나 있습니다.")
        
    # 삼박자가 모두 맞았을 때 최종 보고서 출력
    if is_pop_correct and is_ext_correct and is_dist_correct:
